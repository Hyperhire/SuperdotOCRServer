...
class PGLoss(nn.Layer):
    ...
    def ctcloss(self, f_char, tcl_pos, tcl_mask, tcl_label, label_t):
        ...
        # 한글 dictionary 적용시 여기서 오류날 수 있으니 확인 필요, Lexicon length 정의해서 테스트 진행
        # f_tcl_char = paddle.reshape(f_tcl_char, [-1, 64, 37])  # len(Lexicon_Table)
        # f_tcl_char_fg, f_tcl_char_bg = paddle.split(f_tcl_char, [36, 1], axis=2)
        f_tcl_char = paddle.reshape(f_tcl_char, [-1, 64, self.pad_num+1])  # len(Lexicon_Table)행+1, 원래 37(영문26+숫자10+패딩1)
        ...
        # tcl_mask_fg = paddle.expand(x=tcl_mask, shape=[b, c, self.Lexicon_len * l])
