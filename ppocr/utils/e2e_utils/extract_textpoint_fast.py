...
def ctc_decoder_for_image(gather_info_list,
    ...
    for gather_info in gather_info_list:
        if len(gather_info) < 1: #pts_num(=6)이 들어가는 부분
            continue
        ...
        if len(dst_str_readable) < 1: # 원래 2였음
            continue
...
def restore_poly(instance_yxs_list, seq_strs, p_border, ratio_w, ratio_h, src_w,
    ...
    for yx_center_line, keep_str in zip(instance_yxs_list, seq_strs):
        if len(keep_str) < 1: # 1개짜리 날리는 부분(원래 2였던 것 1로 수정)
...
def generate_pivot_list_fast(p_score,
    ...
    if instance_count > 0:
        for instance_id in range(1, instance_count):
            ...
            if len(pos_list) < 1: # 여기도 원래 3이었음, skeleton_map의 instance_label_map이 3개 포인트 이하이면 없애는 부분
                continue
